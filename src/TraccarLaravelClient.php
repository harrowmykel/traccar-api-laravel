...
class TraccarLaravelClient
{
    ...
    {
        $auth_type = config("traccar.servers.$account.auth_type");
        $base_url = config("traccar.servers.$account.url");
        if ($auth_type === 'email') {
            $email = config("traccar.servers.$account.username");
            $password = config("traccar.servers.$account.password");
            $this->client = TraccarApi::withEmailPassword($email, $password)->setBaseUrl($base_url);
        } elseif ($auth_type === 'token') {
            $auth_token = config("traccar.servers.$account.auth_token");
